...
def summarize_draft(draft_dir: str) -> DraftSummary:
    ...
    if not tracks:
        ...
        for i, tr in enumerate(tracks):
            ...
            ttype = _track_type_zh(tr.get("type", "?"))
...
def _load_draft_payload(dpath: str) -> Dict[str, Any]:
    """后台线程：读取草稿 JSON、解析素材路径并汇总详情（避免阻塞 UI）。"""
    ...
    media_warn: Optional[str] = None
    if summary.content_ok and not encrypted and isinstance(summary.content, dict):
        raw_timeline = summary.content
        _normalize_draft_material_paths(raw_timeline, dpath)
        media_warn = _draft_unresolved_media_summary(raw_timeline)
    return {
        "summary": summary,
        "encrypted": encrypted,
        "raw_timeline": raw_timeline,
        "media_warn": media_warn,
    ...
    for tr in content.get("tracks") or []:
        ...
        for i, seg in enumerate(segs):
            ...
            if ttype == "text":
                ...
                label = f"[{_track_type_zh('text')}] {tname} · 片段{i + 1} · {_fmt_tc_us(t0, t1)} · {lab}"
            else:
                mat = stickers_by_id.get(mid) or {}
                cur_rid = _sticker_resource_id_from_material(mat)
                hint = cur_rid[:16] if cur_rid else mid[:8]
                label = f"[{_track_type_zh('sticker')}] {tname} · 片段{i + 1} · {_fmt_tc_us(t0, t1)} · {hint}"
            out.append(
                StyleSegmentRef(
                    track_type=ttype,
                    track_name=tname,
...
def _probe_media_duration_us(path: str) -> Optional[int]:
    """用 ffprobe 读取媒体时长（微秒）。"""
    fp = find_ffprobe()
    path_abs = os.path.abspath(path) if path else ""
    if not fp or not path_abs or not os.path.isfile(path_abs):
        return None
    try:
        proc = subprocess.run(
            [
...
_DRAFT_LIST_COLORS: Dict[str, Tuple[Tuple[str, str], Tuple[str, str]]] = {
    ...
    "template": (("gray82", "gray24"), ("gray72", "gray34")),
    "selected": (("#3B8ED0", "#1F538D"), ("#36719F", "#14375E")),
}


def _draft_list_item_wraplength(
    *,
    indent: int,
    reserved_right: int = 0,
    panel_width: Optional[int] = None,
) -> int:
    """左侧栏草稿/模板名换行宽度；panel_width 为 ScrollableFrame 实际宽度。"""
    base_w = 236
...
def _make_draft_list_click_box(
    ...
    *,
    ...
    box._title_lbl = lbl  # type: ignore[attr-defined]
    box._wrap_indent = 0  # type: ignore[attr-defined]
    box._wrap_reserved = 0  # type: ignore[attr-defined]
...
def _track_type_zh(ttype: Any) -> str:
    key = str(ttype or "").strip().lower()
    if not key:
        return "?"
    return _TRACK_TYPE_ZH.get(key, str(ttype))
...
def _style_segment_ref_from_timeline(
    ...
    if ttype == "text":
        ...
        lab = _timeline_segment_label(seg, materials)
        label = f"[{_track_type_zh('text')}] {tname} · 片段{segment_index_raw + 1} · {_fmt_tc_us(t0, t1)} · {lab}"
    else:
        mat = {}
        for m in materials.get("stickers") or []:
            if isinstance(m, dict) and str(m.get("id")) == mid:
        ...
        hint = cur_rid[:16] if cur_rid else mid[:8]
        label = f"[{_track_type_zh('sticker')}] {tname} · 片段{segment_index_raw + 1} · {_fmt_tc_us(t0, t1)} · {hint}"
...
_DRAFTPATH_PLACEHOLDER_RE = re.compile(
...
def _is_preview_image_path(path: str) -> bool:
    return os.path.splitext(str(path or ""))[1].lower() in _PREVIEW_IMAGE_EXTS


def _find_basename_in_draft_dir(draft_dir: str, basename: str, *, max_depth: int = 6) -> str:
    """在草稿目录内按文件名递归查找（模板 zip 目录层级不一致时的兜底）。"""
    name = (basename or "").strip()
    if not name or not draft_dir or not os.path.isdir(draft_dir):
        return ""
    root = os.path.abspath(draft_dir)
    ...
    try:
        for dirpath, _dirnames, filenames in os.walk(root):
            ...
            if name in filenames:
                hit = os.path.normpath(os.path.join(dirpath, name))
                if not best or len(hit) < len(best):
                    best = hit
    except OSError:
        return ""
    return best
...
def _resolve_draft_media_path(
    raw_path: str,
    draft_dir: str,
    *,
    basename_cache: Optional[Dict[str, str]] = None,
) -> str:
    """将剪映 materials.path 中的 draftpath 占位符解析为草稿目录内真实文件。"""
    raw = (raw_path or "").strip()
    if not raw:
        return ""
    draft_dir = os.path.abspath(draft_dir)
    p = raw.replace("/", os.sep).replace("\\", os.sep)
    if os.path.isabs(p) and os.path.isfile(p):
        return os.path.normpath(p)
    direct = os.path.join(draft_dir, p)
    if os.path.isfile(direct):
        return os.path.normpath(direct)

    def _cached_basename_lookup(basename: str) -> str:
        if not basename:
            return ""
        if basename_cache is not None and basename in basename_cache:
            return basename_cache[basename]
        walked = _find_basename_in_draft_dir(draft_dir, basename)
        if basename_cache is not None:
            basename_cache[basename] = walked
        return walked

    m = _DRAFTPATH_PLACEHOLDER_RE.search(p)
    if m:
        uuid = m.group(1)
        suffix = p[m.end() :].lstrip("\\/")
        basename = os.path.basename(suffix) if suffix else os.path.basename(p)
        candidates = []
        if suffix:
            candidates.append(os.path.join(draft_dir, suffix))
            candidates.append(os.path.join(draft_dir, uuid, suffix))
        if basename:
            candidates.extend(
                [
                    os.path.join(draft_dir, "videos", basename),
                    os.path.join(draft_dir, uuid, "videos", basename),
                    os.path.join(draft_dir, basename),
                ]
            )
        seen: set[str] = set()
        for cand in candidates:
            ...
            if os.path.isfile(norm):
                return norm
        if basename:
            walked = _cached_basename_lookup(basename)
            if walked:
                return walked
    basename = os.path.basename(p)
    if basename:
        for sub in ("videos", "audio", "audios", "images", ""):
            cand = os.path.join(draft_dir, sub, basename) if sub else os.path.join(draft_dir, basename)
...
def _normalize_draft_material_paths(content: Dict[str, Any], draft_dir: str) -> None:
    """加载草稿后，把 materials 里的占位 path 改写成可读的本地绝对路径。"""
    materials = content.get("materials")
    if not isinstance(materials, dict) or not draft_dir:
        return
    draft_abs = os.path.abspath(draft_dir)
    content["_draft_resolve_dir"] = draft_abs
    materials["_draft_resolve_dir"] = draft_abs
    basename_cache: Dict[str, str] = {}
    for key in ("videos", "audios", "images", "photos"):
        ...
        for mat in items:
            if not isinstance(mat, dict):
                continue
            raw = (mat.get("path") or "").strip()
            if not raw:
                continue
            mat["path"] = _resolve_draft_media_path(
                raw, draft_abs, basename_cache=basename_cache
            )
...
def _local_media_path_for_segment(seg: Dict[str, Any], materials: Dict[str, Any]) -> str:
    ...
    if not mid or not isinstance(materials, dict):
        return ""
    draft_dir = (materials.get("_draft_resolve_dir") or "").strip()
        ...
        for m in materials.get(key) or []:
            if isinstance(m, dict) and m.get("id") == mid:
                ...
                if not p:
                    return ""
                p = p.replace("/", os.sep)
                if draft_dir and _DRAFTPATH_PLACEHOLDER_RE.search(p):
                    p = _resolve_draft_media_path(p, draft_dir)
                return p
    return ""


def _draft_unresolved_media_summary(content: Dict[str, Any]) -> Optional[str]:
    """素材 path 仍为占位符或本地文件不存在时，返回预览提示文案。"""
    materials = content.get("materials")
    if not isinstance(materials, dict):
        return None
    missing: List[str] = []
    for key in ("videos", "audios"):
        for mat in materials.get(key) or []:
            if not isinstance(mat, dict):
                continue
            p = (mat.get("path") or "").strip()
            if not p:
                continue
            local = p.replace("/", os.sep)
            if _DRAFTPATH_PLACEHOLDER_RE.search(local) or not os.path.isfile(local):
                missing.append(os.path.basename(local) or local[:48])
    if not missing:
        return None
    shown = ", ".join(missing[:5])
    extra = f" 等 {len(missing)} 个" if len(missing) > 5 else ""
    return (
        f"【预览提示】以下素材未在草稿目录中找到，画面/音频可能无法预览：{shown}{extra}\n"
        "（若从服务器下载的模板 zip 未包含 videos/ 等媒体文件，需联系模板提供方补全。）"
    )
...
def _ffmpeg_hybrid_seek_args(path_abs: str, sec: float) -> List[str]:
    """混合 seek：粗定位在 -i 前（快），余量在 -i 后（准），避免关键帧落到片段开头。"""
    if _is_preview_image_path(path_abs):
        return ["-i", path_abs]
    if sec < 0.001:
        return ["-i", path_abs]
...
def _ffplay_audio_seek_args(path_abs: str, sec: float) -> List[str]:
    return _ffmpeg_hybrid_seek_args(path_abs, sec)


def _ffplay_audio_args(layer: PreviewAudioLayer, path_abs: str, sec: float) -> List[str]:
    args: List[str] = ["-nodisp", "-autoexit", "-hide_banner"]
...
def kill_playback_audio_procs(procs: Optional[List[subprocess.Popen]]) -> None:
    alive = [p for p in (procs or []) if p is not None and p.poll() is None]
    if not alive:
        return
    # ffmpeg→ffplay 管道：先停消费者(ffplay)再停生产者，避免 pause 时 terminate 卡住 UI
...
# ffplay 周期性刷新 stderr 状态行；M-A 播放时间 + aq>0 表示音频已在输出
_FFPLAY_AUDIO_PLAYING_RE = re.compile(
...
def _ffplay_stderr_audio_ma_sec(text: str) -> Optional[float]:
    m = _FFPLAY_AUDIO_PLAYING_RE.search(text)
    if not m:
        return None
    try:
        ma = float(m.group(1))
        ...
        return None
    if ma < PREVIEW_AUDIO_READY_MIN_PLAY_SEC:
        return None
    return ma
...
def watch_ffplay_audio_output_ready(
    ...
    *,
    ...
    def _fire(ma_sec: float = 0.0) -> None:
        if fired.is_set():
            return
    ...
    timer = threading.Timer(timeout_sec, lambda: _fire(0.0))
    timer.daemon = True
    timer.start()
...
def _collect_preview_video_slices(content: Dict[str, Any], t0_us: int, t1_us: int) -> List[_PreviewVideoSlice]:
    ...
    for i in range(len(bounds) - 1):
        ...
        if not path or not os.path.isfile(path):
            continue
        if not _is_preview_image_path(path) and not _ffmpeg_input_has_stream(path, "video"):
            continue
...
def _win_embed_child_window(
    ...
    *,
    ...
    if not user32.IsWindow(child_hwnd) or not user32.IsWindow(parent_hwnd):
        raise OSError("invalid hwnd for embed")
    ...
    _win_disable_embed_keyboard_focus(child_hwnd)


def _win_disable_embed_keyboard_focus(child_hwnd: int) -> None:
    """嵌入 ffplay 后不抢键盘焦点，空格等快捷键仍由 Tk 处理。"""
    ...
    try:
        exstyle = user32.GetWindowLongW(child_hwnd, gwl_exstyle)
        user32.SetWindowLongW(child_hwnd, gwl_exstyle, exstyle | ws_ex_noactivate)
    except OSError:
        pass
...
def _win_post_space_key(hwnd: int) -> bool:
    """向嵌入的 ffplay 窗口发送空格（ffplay 默认空格暂停/继续）。"""
    ...
    try:
        ...
        if not user32.IsWindow(int(hwnd)):
            return False
        vk = 0x20
        user32.PostMessageW(int(hwnd), 0x0100, vk, 0)
        user32.PostMessageW(int(hwnd), 0x0101, vk, 0)
        return True
    except Exception:
        return False
...
def spawn_merged_preview_player(
    ...
    *,
    ...
    seek = max(0.0, float(start_sec))
    path_abs = os.path.abspath(merge_path)
    if accurate_seek and seek >= 0.05:
        cmd.extend(["-i", path_abs, "-ss", f"{seek:.3f}"])
    else:
        if seek >= 0.05:
            cmd.extend(["-ss", f"{seek:.3f}"])
        cmd.extend(["-i", path_abs])
...
def _warm_video_thumbnail_strip(
    ...
    *,
    ...
    cmd: List[str] = [
        ...
        "-loglevel",
        "error",
        *_ffmpeg_hybrid_seek_args(path_abs, start_sec),
    ]
...
def warm_preview_play_ahead(
    ...
    _start_warm_path(
        ...
        cache,
        start_sec=start_sec,
        duration_sec=PREVIEW_PLAY_WARM_AHEAD_SEC + 1.0,
    )
...
def fetch_play_start_preview_frame(
    plan: PreviewPlan,
    thumb_cache: _ThumbnailStripCache,
    ...
    if hit:
        return hit
    if thumb_cache.has_bucket(top.path, top.source_us):
        return thumb_cache.nearest(top.path, top.source_us)
    return None
...
def fetch_scrub_frame_fast(
    ...
    *,
    ...
    if not ff or not os.path.isfile(layer.path):
        return None
    if frame_cache is not None and not skip_cache:
    ...
    cmd = [
        ...
        "-loglevel",
        "error",
        *_ffmpeg_hybrid_seek_args(path_abs, sec),
        "-frames:v",
        "1",
        "-an",
        "-vf",
...
def fetch_local_mp4_frame_ppm(
    path: str,
    sec: float,
    *,
    max_width: int = PREVIEW_MAX_WIDTH,
) -> Optional[bytes]:
    """从本地 MP4 指定时间抽一帧（暂停时冻结画面）。"""
    ff = find_ffmpeg()
    ...
    cmd = [
        ...
        "-loglevel",
        "error",
        *_ffmpeg_hybrid_seek_args(path_abs, max(0.0, float(sec))),
        "-frames:v",
        "1",
        "-an",
        "-vf",
...
def prefetch_playback_ahead_frames(
    content: Dict[str, Any],
    start_us: int,
    frame_cache: _PreviewFrameCache,
    *,
    ...
    """播放起跑前同步预取后续若干缩略图步长处的帧，避免 arm 后音频先行、画面卡在暂停帧。"""
    total_us = _timeline_end_us(content)
    if total_us <= 0:
        return
...
def populate_timeline_panel(
    ...
    *,
    ...
    def on_select_track(ti: int) -> None:
        ...
        tname = "" if nm_raw is None else (nm_raw if isinstance(nm_raw, str) else str(nm_raw))
        tname = tname.strip()
        ttype_zh = _track_type_zh(ttype)
        sel["track_summary"] = f"{tname or ttype_zh} [{ttype_zh}] · 共 {nseg} 个片段"
        _set_status(
            f"已选轨道：{tr.get('name')} [{_track_type_zh(tr.get('type'))}] · 共 {nseg} 个片段",
            replace_highlight="",
        )
    ...
    def _on_playhead_press(event: tk.Event) -> Optional[str]:
        ...
        if cx < label_w:
            return None
        target_us = _us_from_canvas_x(cx)
        is_playing = rs.get("_preview_is_playing")
        if callable(is_playing) and is_playing():
            ...
            inv = rs.get("_invalidate_playback_seek")
            if callable(inv):
                try:
                    inv(int(target_us))
                except Exception:
...
class _DraftImportProgressDialog:
    """模板下载/导入进度（主线程创建，后台线程通过 root.after 更新）。"""

    __slots__ = ("_root", "_win", "_label_var", "_bar", "_indeterminate", "_pulse_after", "_closed", "_destroy_after")

    def __init__(self, parent: Any, *, title: str = "导入草稿") -> None:
        import customtkinter as ctk

        self._root = parent
        self._indeterminate = False
        self._pulse_after: Optional[str] = None
        self._destroy_after: Optional[str] = None
        self._closed = False
        win = ctk.CTkToplevel(parent)
        win.title(title)
        win.transient(parent)
        ...
        try:
            ...
            pass
        self._win = win
        self._label_var = ctk.StringVar(value="准备中…")
        ctk.CTkLabel(
            win,
        ...
        self._bar = ctk.CTkProgressBar(win, width=380, height=14)
        self._bar.pack(fill="x", padx=18, pady=(0, 16))
        self._bar.set(0)
    ...
    def _cancel_destroy(self) -> None:
        aid = self._destroy_after
        if aid is not None:
            try:
                self._root.after_cancel(aid)
            except (tk.TclError, ValueError, TypeError):
                pass
            self._destroy_after = None
    ...
    def _start_pulse(self) -> None:
        ...
        def _tick(step: float = 0.04) -> None:
            if self._closed or not self._win_alive():
                self._pulse_after = None
                return
            ...
            try:
                ...
                self._pulse_after = None
                return
            self._pulse_after = self._root.after(80, lambda: _tick(step))

        _tick()

    def update(self, ratio: Optional[float], message: str) -> None:
        if self._closed:
            return
        ...
        def _ui() -> None:
            ...
            self._indeterminate = False
            try:
                self._bar.set(max(0.0, min(1.0, float(ratio))))
            except tk.TclError:
                pass
    ...
    def close(self) -> None:
        ...
        def _ui() -> None:
            ...
            try:
                ...
                pass
            # CustomTkinter 在 CTkToplevel 创建后会 after(5/10ms) deiconify/restore focus；
            # 立即 destroy 会留下对已销毁窗口的回调。先 withdraw 并延迟销毁。
            try:
                win.withdraw()
            except tk.TclError:
                pass
            ...
            self._cancel_destroy()
            try:
                self._destroy_after = self._root.after(30, _destroy)
            except tk.TclError:
                _destroy()
...
def run_app() -> None:
    ...
    template_client: Any = None
    _auth_api_error_message: Any = None
    try:
        from shared.api_config import api_base_url
        ...
        from shared.hot_template_client import HotTemplateClient as _HTC

        _api_base = api_base_url()
        auth_client = _BAC(_api_base)
        template_client = _HTC(_api_base)
    except ImportError:
        auth_client = None
    ...
    left_tab_var = ctk.StringVar(value="drafts")
    left_tab_bar = ctk.CTkFrame(left, fg_color="transparent")
    ...
    def _style_left_tab_btn(btn: Any, *, active: bool) -> None:
        if active:
            btn.configure(fg_color=("gray75", "gray35"), hover_color=("gray70", "gray40"), border_width=0)
        else:
            btn.configure(
    ...
    template_status_var = ctk.StringVar(value="切换到本页后从服务器拉取模板列表")
    ctk.CTkLabel(
        template_toolbar,
        textvariable=template_status_var,
        font=ctk.CTkFont(size=11),
        text_color=("gray45", "gray60"),
        anchor="w",
        wraplength=240,
    ...
    template_list_frame = ctk.CTkScrollableFrame(template_panel, label_text="模板列表", corner_radius=10)
    template_list_frame.pack(fill="both", expand=True, padx=8, pady=(0, 8))
    ...
    def _redraw_template_list_scroll(*, reset_scroll: bool = False) -> None:
        try:
            template_list_frame.update_idletasks()
            pc = getattr(template_list_frame, "_parent_canvas", None)
            if pc is not None:
                pc.update_idletasks()
                bb = pc.bbox("all")
                if bb:
    ...
    def _relayout_left_list_wraplengths() -> None:
        """侧栏宽度变化后只更新标题 wraplength，不重建列表。"""
        try:
            pw = int(list_frame.winfo_width())
        except (tk.TclError, ValueError, TypeError):
            pw = 0
        if pw < 80:
            try:
                pw = int(template_list_frame.winfo_width())
            except (tk.TclError, ValueError, TypeError):
                pw = 0
        if pw < 80:
            return
        for boxes in (draft_buttons, template_buttons, library_buttons):
            for box in boxes:
                ...
                try:
                    lbl.configure(
                        wraplength=_draft_list_item_wraplength(
                            indent=int(getattr(box, "_wrap_indent", 0) or 0),
                            reserved_right=int(getattr(box, "_wrap_reserved", 0) or 0),
                            panel_width=pw,
                        )
                    )
                except tk.TclError:
                    pass
        _redraw_list_scroll()
        _redraw_template_list_scroll()

    _left_list_relayout_after: List[Optional[str]] = [None]

    def _schedule_left_list_relayout(_event: Any = None) -> None:
        aid = _left_list_relayout_after[0]
        if aid is not None:
            try:
                root.after_cancel(aid)
            except (tk.TclError, ValueError, TypeError):
                pass
        _left_list_relayout_after[0] = root.after(120, _relayout_left_list_wraplengths)

    left.bind("<Configure>", _schedule_left_list_relayout, add="+")

    def show_template_summary(node: Any) -> None:
    ...
    def refresh_template_list(*, reset_scroll: bool = False) -> None:
        ...
        nonlocal template_tree_roots
        if template_fetch_busy["on"]:
            return
        for w in template_list_frame.winfo_children():
            w.destroy()
        if not template_client:
        ...
        template_status_var.set("正在从服务器拉取模板…")
        ctk.CTkLabel(template_list_frame, text="加载中…", text_color=("gray45", "gray60")).pack(pady=20)
        _redraw_template_list_scroll(reset_scroll=reset_scroll)
        template_fetch_busy["on"] = True
        ...
        def _bg() -> None:
            ...
            try:
                roots, err = template_client.fetch_list(user_id=uid)
    ...
    ctk.CTkButton(
        ...
        fg_color="transparent",
        border_width=1,
        command=lambda: refresh_template_list(reset_scroll=True),
    ...
    preview_actions_row = ctk.CTkFrame(preview_col, fg_color="transparent")
    preview_actions_row.pack(fill="x", padx=10, pady=(0, 6))
    preview_state: Dict[str, Any] = {
        ...
        "play_prime_gen": 0,
        "play_mode": "live",
        "draft_preview_mp4": False,
        "draft_preview_mp4_end_us": None,
        "merge_path": None,
        "merge_t0_us": 0,
        ...
        "play_pause_timeline_us": None,
        "cloud_preview_fetch_busy": False,
        "cloud_preview_fetch_gen": 0,
        "pending_timeline_preview_after_load": None,
        "merge_ffplay_frozen": False,
    }

    def _current_timeline_content() -> Optional[Dict[str, Any]]:
        raw = timeline_content_cache[0]
        return raw if isinstance(raw, dict) else None

    def _cancel_cloud_preview_fetch() -> None:
        ...
        preview_state["cloud_preview_draft_name"] = None
        preview_state["template_preview_node_id"] = None

    def _on_draft_cloud_preview_ready(mp4_path: str, folder_name: str, us: int) -> None:
        content = _current_timeline_content() or {}
        try:
            preview_info_var.set(f"草稿 · {folder_name} · 云端预览")
        except Exception:
            pass
        _play_draft_preview_mp4(content, us, mp4_path=mp4_path)

    def _on_draft_cloud_preview_fail(msg: str, folder_name: str, us: int) -> None:
        content = _current_timeline_content()
        if content is None:
            preview_state["pending_timeline_preview_after_load"] = {
                "us": int(us),
                "folder_name": folder_name,
                "cloud_unavailable": True,
            }
            _preview_show_message(f"（{msg}，草稿加载完成后将尝试时间轴预览）")
            try:
                preview_info_var.set(f"草稿 · {folder_name} · {msg}")
            except Exception:
                pass
            return
        try:
            preview_info_var.set(f"草稿 · {folder_name} · {msg} · 时间轴预览")
        except Exception:
            pass
        _continue_draft_timeline_preview_playback(
            content, us, cloud_unavailable=True, prefer_timeline=True
        )

    def _maybe_resume_pending_timeline_preview() -> None:
        pending = preview_state.get("pending_timeline_preview_after_load")
        if not isinstance(pending, dict):
            return
        folder_name = (replace_state.get("timeline_draft_name") or "").strip()
        if folder_name != str(pending.get("folder_name") or "").strip():
            return
        content = _current_timeline_content()
        if content is None:
            return
        preview_state["pending_timeline_preview_after_load"] = None
        us = int(pending.get("us") or 0)
        _continue_draft_timeline_preview_playback(
            content,
            us,
            cloud_unavailable=bool(pending.get("cloud_unavailable")),
            prefer_timeline=True,
        )

    def _current_draft_dir() -> str:
        d = (replace_state.get("timeline_draft_dir") or "").strip()
        if d and os.path.isdir(d):
            return os.path.abspath(d)
        content = timeline_content_cache[0]
        if isinstance(content, dict):
            d2 = (content.get("_draft_resolve_dir") or "").strip()
            if d2 and os.path.isdir(d2):
                return os.path.abspath(d2)
        return ""
    ...
    def _playback_end_us(content: Dict[str, Any]) -> int:
        ...
        if preview_state.get("draft_preview_mp4"):
            ...
            if mp4_end:
                return int(mp4_end)
        return _timeline_end_us(content)
    ...
    def _show_merge_ffplay_ui() -> None:
        ...
        # Canvas 重写了 lift/tkraise（=tag_raise），提升控件层级须走 Misc 基类
        try:
            tk.Misc.tkraise(preview_sub_overlay)
        except tk.TclError:
            pass
    ...
    def _cancel_merge_ffplay_embed_retries() -> None:
        aid = preview_state.get("merge_embed_after_id")
        if aid is not None:
            try:
                root.after_cancel(aid)
            except (tk.TclError, ValueError, TypeError):
                pass
            preview_state["merge_embed_after_id"] = None

    def _bump_merge_ffplay_generation() -> None:
        """使 embed 重试 / 旧 ffplay 回调失效，避免暂停后仍操作已销毁窗口。"""
        preview_state["merge_ffplay_token"] = int(preview_state.get("merge_ffplay_token") or 0) + 1
        ...
        _cancel_merge_ffplay_embed_retries()
    ...
    def _attach_playback_audio_ready_watch(
        ...
        *,
        ...
        def _on_ready(_ma_sec: float = 0.0) -> None:
            def _ui() -> None:
                ...
                # merge/ffplay 须先嵌入预览区，再由 poll 统一 arm 时钟
    ...
    def _start_merged_preview_ffplay(*, start_sec: float = 0.0, force: bool = False) -> bool:
        ...
        if not merge_path or not os.path.isfile(str(merge_path)):
            return False
        start_sec = max(0.0, float(start_sec))
        use_accurate_seek = bool(preview_state.get("draft_preview_mp4"))
        sess = preview_state.get("merge_ffplay_session")
        last_sec = preview_state.get("merge_ffplay_start_sec")
        if (
            not force
            and isinstance(sess, _MergedPreviewFfplaySession)
            and sess.alive()
            and last_sec is not None
            and abs(float(last_sec) - start_sec) < 0.04
        ):
        ...
        if sys.platform == "win32" and player.spawn(
            ...
            if use_accurate_seek:
                preview_state["play_audio_output_ready"] = False
            _schedule_merge_ffplay_embed_retries()
            return True
    ...
    def _try_embed_merged_preview_ffplay() -> bool:
        ...
        _refocus_preview_keyboard()
        return True

    def _schedule_merge_ffplay_embed_retries(*, attempts: int = 30) -> None:
        """spawn 后尽快把 ffplay 嵌入预览区，避免独立窗口长时间弹出。"""
        _cancel_merge_ffplay_embed_retries()
        gen = int(preview_state.get("merge_ffplay_token") or 0)

        def _try(left: int) -> None:
            preview_state["merge_embed_after_id"] = None
            if not _preview_is_playing():
                return
            if int(preview_state.get("merge_ffplay_token") or 0) != gen:
                return
            if preview_state.get("merge_ffplay_embedded") or preview_state.get("merge_ffplay_fallback"):
                return
            try:
                if _try_embed_merged_preview_ffplay():
                    return
            except Exception:
                pass
            if left > 0:
                preview_state["merge_embed_after_id"] = root.after(
                    40, lambda n=left - 1: _try(n)
                )

        preview_state["merge_embed_after_id"] = root.after(0, lambda: _try(attempts))

    def _refocus_preview_keyboard() -> None:
        """嵌入 ffplay 后把键盘焦点还给主窗口，避免空格无法暂停。"""
        try:
            root.focus_force()
            preview_col.focus_set()
        except tk.TclError:
            pass
    ...
    def _playback_video_ready(playhead_us: int) -> bool:
        """视频已绘制到 canvas 且与播放头对齐，时间轴可以起跑。"""
        ...
        if plan.videos and display is None:
            return False
        if display is not None and abs(int(display) - int(playhead_us)) > SCRUB_THUMB_STEP_US:
            return False
        return True
    ...
    def _try_start_playback_clock() -> None:
        ...
        if use_merge and preview_state.get("draft_preview_mp4"):
            ...
            if abs(cur - us) > 1000:
                ...
                if _start_merged_preview_ffplay(
                    start_sec=max(0.0, us / 1_000_000.0),
                    force=True,
                ):
                    _schedule_playback_arm(us)
                return
        if not use_merge and preview_state.get("play_mode") == "scrub":
            if not preview_state.get("play_prime_ready"):
                return
        elif not use_merge and not _playback_video_ready(us):
            return
        content = timeline_content_cache[0]
        hits = (
            _playback_audio_hits(content, us) if isinstance(content, dict) else ()
        )
        if not use_merge and hits and not preview_state.get("play_audio_output_ready"):
            return
        ...
        if use_merge and preview_state.get("draft_preview_mp4"):
            ...
            preview_state["play_wall_t0"] = time.time() - ma + PREVIEW_AUDIO_OUTPUT_LATENCY_SEC
        else:
    ...
    def _poll_playback_arm() -> None:
        preview_state["play_clock_arm_after_id"] = None
        if int(preview_state.get("playback_arm_poll_gen") or 0) != int(
            preview_state.get("playback_arm_gen") or 0
        ):
            return
        if not _preview_is_playing():
            return
        ...
        if use_merge:
            ...
            last_sec = preview_state.get("merge_ffplay_start_sec")
            seek_mismatch = (
                last_sec is not None and abs(float(last_sec) - file_sec) >= 0.04
            )
            procs = preview_state.get("play_audio_procs") or []
            if not procs or seek_mismatch:
                if not _start_merged_preview_ffplay(
                    start_sec=file_sec,
                    force=bool(
                        seek_mismatch
                        or preview_state.get("draft_preview_mp4")
                        or not procs
                    ),
                ):
                    started_at = float(preview_state.get("play_audio_arm_started_at") or time.time())
            ...
            if not preview_state.get("merge_ffplay_fallback") and not preview_state.get("merge_ffplay_embedded"):
                if not _try_embed_merged_preview_ffplay():
                    ...
                    return
            if preview_state.get("draft_preview_mp4"):
                if not preview_state.get("merge_ffplay_embedded"):
                    if preview_state.get("merge_ffplay_fallback"):
                        _repoll_playback_arm()
                        return
                    if not _try_embed_merged_preview_ffplay():
    ...
    def _schedule_playback_arm(playhead_us: int) -> None:
        _cancel_playback_arm()
        gen = int(preview_state.get("playback_arm_gen") or 0) + 1
        preview_state["playback_arm_gen"] = gen
        preview_state["playback_arm_poll_gen"] = gen
        preview_state["play_arm_us"] = int(playhead_us)
    ...
    def _playback_tick_still_valid(tick_gen: int) -> bool:
        return _preview_is_playing() and tick_gen == int(
            preview_state.get("play_tick_gen") or 0
        )
    ...
    def _merge_preview_file_sec_at_timeline(timeline_us: int) -> float:
        t0 = int(preview_state.get("merge_t0_us") or 0)
        return max(0.0, (int(timeline_us) - t0) / 1_000_000.0)

    def _freeze_merge_ffplay_to_canvas(pause_us: int) -> bool:
        """停止 ffplay 并在 canvas 显示暂停时刻的帧（SDL 嵌入窗口无法可靠 PostMessage 暂停）。"""
        merge_path = preview_state.get("merge_path")
        if not merge_path or not os.path.isfile(str(merge_path)):
            return False
        content = _current_timeline_content()
        plan = (
            build_scrub_preview_plan(content, int(pause_us))
            if isinstance(content, dict)
            else PreviewPlan(
                playhead_us=int(pause_us),
                videos=(),
                texts=(),
                sticker_count=0,
                info="",
        ...
        sec = _merge_preview_file_sec_at_timeline(int(pause_us))
        ppm = fetch_local_mp4_frame_ppm(str(merge_path), sec, max_width=PREVIEW_MAX_WIDTH)
        _stop_playback_audio()
        if ppm:
            try:
                _apply_preview_image(ppm, plan, int(preview_state.get("gen", 0)), subtitle_us=int(pause_us))
            except Exception:
                pass
        preview_state["merge_ffplay_frozen"] = True
        preview_state["play_prime_ready"] = ppm is not None
        return True

    def _resume_frozen_merge_ffplay(us: int) -> bool:
        merge_path = preview_state.get("merge_path")
        if not merge_path or not os.path.isfile(str(merge_path)):
            return False
        content = _current_timeline_content() or {}
        us = int(us)
        preview_state["merge_ffplay_frozen"] = False
        preview_state["play_tick_gen"] = int(preview_state.get("play_tick_gen") or 0) + 1
        ...
        if preview_state.get("play_mode") == "library":
            ...
            _preview_play_tick()
            return True
        _begin_draft_preview_mp4_play(content, us, mp4_path=str(merge_path))
        return True

    def _merge_ffplay_preview_active() -> bool:
        if not (
            preview_state.get("draft_preview_mp4")
            or preview_state.get("play_mode") in ("merge", "library")
        ):
            return False
        sess = preview_state.get("merge_ffplay_session")
        return isinstance(sess, _MergedPreviewFfplaySession) and sess.alive()

    def _can_resume_merge_preview(us: int) -> bool:
        ...
        if not merge_path or not os.path.isfile(str(merge_path)):
            return False
        return _merge_ffplay_preview_active() or bool(preview_state.get("merge_ffplay_frozen"))

    def _stop_playback_audio(*, keep_merge_ffplay: bool = False) -> None:
        sess = preview_state.get("merge_ffplay_session")
        session_proc = (
            sess.proc
            if isinstance(sess, _MergedPreviewFfplaySession) and sess.proc is not None
            else None
        )
        procs = list(preview_state.get("play_audio_procs") or [])
        if keep_merge_ffplay and session_proc is not None and session_proc.poll() is None:
            extra = [
                p
                for p in procs
                if p is not None and p is not session_proc and p.poll() is None
            ]
            if extra:
                kill_playback_audio_procs(extra)
            preview_state["play_audio_procs"] = [session_proc]
            preview_state["play_audio_proc"] = session_proc
            return
    ...
    def _clear_playback_session() -> None:
        preview_state["playback_session_active"] = False
        preview_state["play_pause_timeline_us"] = None

    def _sync_playhead_for_playback(us: int) -> None:
        """统一写入播放头，并同步预览/播放起跑位置。"""
        us = int(us)
        replace_state["playhead_us"] = us
        timeline_select["playhead_us"] = us
        preview_state["pending_us"] = us
        preview_state["play_start_us"] = us
        preview_state["play_arm_us"] = us

    def _playback_seeked_since_pause(playhead_us: int) -> bool:
        pause_us = preview_state.get("play_pause_timeline_us")
        if pause_us is None:
            return False
        return abs(int(playhead_us) - int(pause_us)) > SCRUB_THUMB_STEP_US // 4

    def _invalidate_playback_arm_polls() -> None:
        preview_state["playback_arm_gen"] = int(preview_state.get("playback_arm_gen") or 0) + 1
        _cancel_playback_arm()

    def _invalidate_playback_seek(playhead_us: int) -> None:
        """暂停后点击/拖动时间轴：从当前头重新播，不再沿用暂停点。"""
        us = int(playhead_us)
        _sync_playhead_for_playback(us)
        if _playback_seeked_since_pause(us):
    ...
    def _try_resume_paused_preview() -> bool:
        """空格续播：保留暂停前的预览画面，不重新拉云端或 prime。"""
        ...
        us = int(pause_us)
        content = timeline_content_cache[0]
        if _can_resume_merge_preview(us):
            return _resume_frozen_merge_ffplay(us)
        if not isinstance(content, dict) or not _can_resume_playback(content, us):
            return False
        preview_state["play_tick_gen"] = int(preview_state.get("play_tick_gen") or 0) + 1
        preview_state["playing"] = True
        _preview_play_btn_set("⏸")
        _begin_scrub_play(content, us, video_ready=True, resume=True)
        return True

    def _pause_preview_playback() -> None:
        """暂停：停时钟与独立音频，保留当前预览画面（ffplay 冻结帧或 canvas 帧）。"""
        pause_us = _playback_timeline_us()
        if pause_us is None:
            pause_us = _current_playhead_us()
        pause_us = int(pause_us)
        preview_state["play_tick_gen"] = int(preview_state.get("play_tick_gen") or 0) + 1
        ...
        _invalidate_playback_arm_polls()
        preview_state["play_audio_ready_gen"] = int(
            preview_state.get("play_audio_ready_gen") or 0
        ) + 1
        ...
        if not _merge_ffplay_preview_active():
            _stop_playback_audio()
        preview_state["play_pause_timeline_us"] = pause_us
        preview_state["playback_session_active"] = True
        preview_state["play_display_us"] = pause_us
        preview_state["play_wall_t0"] = None
        _sync_playhead_for_playback(pause_us)
        set_ph = replace_state.get("_set_playhead")
        if callable(set_ph):
            try:
                set_ph(pause_us, notify=False, redraw=True)
            except TypeError:
                try:
                    set_ph(pause_us, notify=False)
                except Exception:
                    pass
            except Exception:
                pass
        if _merge_ffplay_preview_active():
            _freeze_merge_ffplay_to_canvas(pause_us)
        preview_state["merge_ffplay_start_sec"] = None
        if preview_state.get("photo") is None and not preview_state.get("merge_ffplay_frozen"):
        ...
        content = timeline_content_cache[0]
        if isinstance(content, dict):
            _preview_sync_timecode(pause_us, content)
        _preview_play_btn_set("▶")
        try:
            root.after(0, _refocus_preview_keyboard)
        except tk.TclError:
            pass
    ...
    def _refresh_playback_subtitles_throttled(timeline_us: int) -> None:
        now_ms = time.time() * 1000.0
        last_ms = float(preview_state.get("play_last_sub_refresh_ms") or 0.0)
        if now_ms - last_ms < PREVIEW_PLAY_SUBTITLE_THROTTLE_MS:
            return
        preview_state["play_last_sub_refresh_ms"] = now_ms
    ...
    def _update_playback_preview_frame(content: Dict[str, Any], timeline_us: int) -> None:
        ...
        if bucket_key is not None and bucket_key == preview_state.get("play_last_instant_bucket"):
            return
        if _try_instant_scrub_preview(plan):
            return
        if not preview_state.get("scrub_busy"):
            _run_fast_preview_fetch(plan, int(timeline_us))
    ...
    def _advance_merge_chunk(timeline_us: int) -> None:
        ...
        if not _start_merged_preview_ffplay(start_sec=file_sec, force=True):
            _stop_preview_playback()
    ...
    def _ensure_scrub_preview_audio(
        ...
        *,
        ...
        if (
            need_resync
            and _preview_is_playing()
            and preview_state.get("play_wall_t0") is not None
        ):
            cur_us = _playback_timeline_us()
            if cur_us is not None:
                preview_state["play_start_us"] = int(cur_us)
                preview_state["play_arm_us"] = int(cur_us)
                preview_state["play_wall_t0"] = time.time()
        if alive:
            _stop_playback_audio()
    ...
    def _current_playhead_us() -> int:
        return int(
            replace_state.get("playhead_us")
            or timeline_select.get("playhead_us")
            or preview_state.get("pending_us")
            or 0
        )
    ...
    def _start_playback_preview_prime(content: Dict[str, Any], playhead_us: int) -> None:
        ...
        frame_cache = preview_state["frame_cache"]
        warm_preview_play_ahead(content, us, thumb_cache)
        instant = fetch_play_start_preview_frame(plan, thumb_cache, frame_cache)
        if instant:
            _finish_playback_prime(content, us, plan, prime_gen, instant)
            return
        ...
        def _worker() -> None:
            img = fetch_scrub_frame_fast(
                plan, frame_cache=frame_cache, skip_cache=True
            )

            def _ui() -> None:
                _finish_playback_prime(content, us, plan, prime_gen, img)
    ...
    def _preview_play_tick() -> None:
        ...
        tick_gen = int(preview_state.get("play_tick_gen") or 0)
        preview_state["play_after_id"] = None
        if not _playback_tick_still_valid(tick_gen):
            return
        if preview_state.get("play_mode") == "library":
            tick_ms = 40
            if preview_state.get("play_wall_t0") is None:
            ...
            if (
                ...
                try:
                    ...
                    pass
            if _playback_tick_still_valid(tick_gen):
                preview_state["play_after_id"] = root.after(tick_ms, _preview_play_tick)
            return
        content = timeline_content_cache[0]
        if not isinstance(content, dict):
        ...
        if preview_state.get("play_wall_t0") is None:
            if preview_state.get("play_clock_arm_after_id") is None:
                if preview_state.get("play_mode") == "scrub":
                    preview_state["play_prime_ready"] = True
                    cur = int(_current_playhead_us())
                    preview_state["play_arm_us"] = cur
                    preview_state["play_start_us"] = cur
                    if not (
                        _playback_audio_hits(content, cur)
                        if isinstance(content, dict)
                        else ()
                    ):
                        preview_state["play_audio_output_ready"] = True
                    ...
                    if preview_state.get("play_wall_t0") is None:
                        ...
                        if (time.time() - started) * 1000.0 >= PREVIEW_PLAY_SYNC_TIMEOUT_MS:
                            preview_state["play_audio_output_ready"] = True
                            _try_start_playback_clock()
                        if preview_state.get("play_wall_t0") is None:
        ...
        try:
            ...
            elif preview_state.get("play_mode") == "merge":
                procs = preview_state.get("play_audio_procs") or []
                if procs and not any(p is not None and p.poll() is None for p in procs):
                    _stop_preview_playback()
                    return
    ...
    def _can_resume_playback(content: Dict[str, Any], us: int) -> bool:
        ...
        if not preview_state.get("playback_session_active"):
            return False
        if _can_resume_merge_preview(int(us)):
            return True
    ...
    def _poll_play_sync_start() -> None:
        ...
        if (time.time() - started_at) * 1000.0 >= PREVIEW_PLAY_SYNC_TIMEOUT_MS:
            ...
            preview_state["play_audio_output_ready"] = True
            _try_sync_scrub_play_start()
        if preview_state.get("play_wall_t0") is None and _preview_is_playing():
            _schedule_play_sync_poll()
    ...
    def _arm_scrub_playback_sync(
        ...
        if (
            _preview_is_playing()
            and preview_state.get("play_mode") == "scrub"
            and preview_state.get("play_wall_t0") is not None
        ):
            return
        us = int(us)
    ...
    def _begin_draft_preview_mp4_play(
        ...
        *,
        ...
        _sync_playhead_for_playback(us)
        timeline_total_us = _timeline_end_us(content)
        mp4_dur_us = _probe_media_duration_us(mp4)
        preview_state["draft_preview_mp4_end_us"] = mp4_dur_us
        end_us = int(mp4_dur_us) if mp4_dur_us else int(timeline_total_us)
        _cancel_playback_arm()
        _stop_merged_preview_ffplay()
        preview_state["play_mode"] = "merge"
        preview_state["draft_preview_mp4"] = True
        preview_state["merge_path"] = mp4
        preview_state["merge_t0_us"] = 0
        preview_state["merge_chunk_t0_us"] = 0
        preview_state["merge_chunk_end_us"] = end_us
        preview_state["merge_next_path"] = None
        preview_state["merge_next_t0_us"] = None
        preview_state["merge_next_end_us"] = None
        preview_state["merge_waiting_next"] = False
        ...
        if not _start_merged_preview_ffplay(start_sec=start_sec, force=True):
            _stop_preview_playback()
            _preview_show_message("无法播放预览 MP4（请确认已安装 ffplay，或先拉取云端预览）")
            return
        try:
    ...
    def _continue_draft_timeline_preview_playback(
        ...
        *,
        ...
        if not isinstance(content, dict):
            ...
            return
        us = int(_current_playhead_us())
        draft_mp4 = "" if prefer_timeline else _draft_preview_mp4_path()
        plan = build_scrub_preview_plan(content, us)
        ...
        seeked_since_pause = _playback_seeked_since_pause(us)
        if seeked_since_pause:
            _clear_playback_session()
            preview_state["merge_ffplay_start_sec"] = None
        ...
        resume = _can_resume_playback(content, us) and not seeked_since_pause
    ...
    def _toggle_preview_playback() -> None:
        ...
        if left_tab_var.get() == "templates":
            ...
            if tpl_node is not None and getattr(tpl_node, "has_preview", False):
                ...
                if not lib_path or not os.path.isfile(str(lib_path)):
                    ...
                    return
        if preview_state.get("play_mode") == "library" and left_tab_var.get() != "drafts":
        ...
        if left_tab_var.get() == "drafts" and folder_name:
            abs_url = _cloud_preview_abs_url_for_draft(folder_name)
            if abs_url:
                _fetch_cloud_preview_mp4(
                    abs_url,
                    cache_key=_draft_cloud_preview_cache_key(folder_name),
                    token_key="cloud_preview_draft_name",
                    token_value=folder_name,
                    status_label=f"草稿 · {folder_name}",
                    on_ready=lambda path, fn=folder_name, u=us: _on_draft_cloud_preview_ready(
                        path, fn, u
                    ),
                    on_fail=lambda msg, fn=folder_name, u=us: _on_draft_cloud_preview_fail(
                        msg, fn, u
                    ),
                )
                return
            try:
                preview_info_var.set(f"草稿 · {folder_name} · 无法请求云端预览")
            except Exception:
                pass
        _continue_draft_timeline_preview_playback(
            _current_timeline_content(), us, prefer_timeline=True
        )
        return

    def clear_cloud_preview_cache() -> None:
        from tkinter import messagebox

        from shared.hot_template_client import clear_template_preview_cache, template_preview_cache_dir

        cache_dir = template_preview_cache_dir()
        if not messagebox.askyesno(
            "清除预览缓存",
            f"将删除云端预览的本地缓存：\n{cache_dir}\n\n"
            "下次按空格/播放预览时会重新从服务器拉取。是否继续？",
        ):
            return
        _cancel_cloud_preview_fetch()
        count, nbytes = clear_template_preview_cache()
        if count <= 0:
            messagebox.showinfo("清除预览缓存", "当前没有可清除的云端预览缓存。")
            try:
                preview_info_var.set("云端预览缓存为空")
            except Exception:
                pass
            return
        mb = nbytes / (1024.0 * 1024.0)
        size_txt = f"（约 {mb:.1f} MB）" if nbytes > 0 else ""
        messagebox.showinfo("清除预览缓存", f"已删除 {count} 个缓存文件{size_txt}。")
        try:
            preview_info_var.set(f"已清除 {count} 个云端预览缓存{size_txt}")
        except Exception:
            pass
    ...
    ctk.CTkButton(
        ...
        fg_color="transparent",
        border_width=1,
        command=clear_cloud_preview_cache,
    ).pack(anchor="e")
    ...
    def _find_hot_template_node_by_id(template_id: str) -> Any:
        ...
        def _walk(nodes: List[Any]) -> Any:
            for node in nodes:
                if getattr(node, "is_template", False) and str(node.template_id) == target:
                    return node
                if getattr(node, "is_folder", False):
                    hit = _walk(list(node.children or ()))
                    if hit is not None:
    ...
    def _cloud_preview_abs_url_for_draft(folder_name: str) -> str:
        ...
        if node is None and selected_template_id:
            node = _find_hot_template_node_by_id(str(selected_template_id))
        if node is None:
            sel = selected_template_node[0]
            if sel is not None and getattr(sel, "is_template", False):
                node = sel
        ...
        preview_name = folder_name
        if node is not None and getattr(node, "name", ""):
            preview_name = str(node.name)
    ...
    def _draft_cloud_preview_cache_key(folder_name: str) -> str:
        if selected_template_id:
            return f"id:{selected_template_id}"
    ...
    def _fetch_cloud_preview_mp4(
        ...
        *,
        ...
        use_cache = os.path.isfile(cache_path) and os.path.getsize(cache_path) > 1024
        ...
        try:
            ...
            pass
        progress_dlg: Optional[_DraftImportProgressDialog] = None
        if use_cache:
            try:
                preview_info_var.set(f"{status_label} · 使用本地缓存…")
            except Exception:
                pass
        else:
            progress_dlg = _DraftImportProgressDialog(root, title=f"拉取预览 · {status_label}")
            progress_dlg.update(0.0, "正在连接…")
        try:
            root.update_idletasks()
        except tk.TclError:
            pass

        def _report_download(ratio: Optional[float], message: str) -> None:
            if progress_dlg is not None:
                progress_dlg.update(ratio, message)
        ...
        def _bg() -> None:
            ...
            if use_cache:
                ...
                ok, err = fetch_template_preview_mp4(
                    abs_url, cache_path, on_progress=_report_download
                )
    ...
    replace_state["_preview_is_playing"] = _preview_is_playing
    ...
    def _layout_preview_ppm(
        ...
        *,
        ...
        texts = plan.texts
        if subtitle_us is None and _preview_is_playing():
            timeline_us = _playback_timeline_us()
    ...
    def _apply_preview_image(
        ...
        *,
        ...
        preview_state["play_display_us"] = int(plan.playhead_us)
        return _layout_preview_ppm(img_bytes, plan, gen, subtitle_us=subtitle_us, silent=silent)

    def _apply_playback_frame_sync(
        img_bytes: bytes,
        plan: PreviewPlan,
        *,
        subtitle_us: Optional[int] = None,
    ) -> None:
        """播放中同步换帧（不走 root.after 队列，避免音频已走、画面仍停旧帧）。"""
        gen = int(preview_state.get("gen", 0))
        try:
            _apply_preview_image(img_bytes, plan, gen, subtitle_us=subtitle_us)
        except Exception:
            return
        bucket_key = preview_thumb_bucket_key(plan)
    ...
    def _finish_playback_prime(
        ...
        if img_bytes:
            try:
                _apply_preview_image(img_bytes, plan, gen, subtitle_us=us)
            except Exception:
                pass
        bucket_key = preview_thumb_bucket_key(plan)
        if bucket_key is not None:
        ...
        try:
            ...
            pass
        if gen != int(preview_state.get("play_prime_gen", -1)):
            return
        _mark_playback_prime_ready()
        if preview_state.get("play_wall_t0") is not None:
            return
    ...
    def _maybe_warm_scrub_strip(plan: PreviewPlan) -> None:
        ...
        throttle_ms = (
            PREVIEW_PLAY_WARM_THROTTLE_MS
            if _preview_is_playing()
            else PREVIEW_WARM_SCRUB_THROTTLE_MS
        )
    ...
    def _run_fast_preview_fetch(plan: PreviewPlan, us: int) -> None:
        ...
        if playing or scrubbing:
            ...
            preview_state["gen"] = apply_gen
            worker_token = apply_gen
        ...
        def worker() -> None:
            ...
            def ui() -> None:
                ...
                if img:
                    ...
                    _queue_preview_apply(img, plan, apply_gen, subtitle_us=sub_us)
    ...
    def on_playhead_changed(playhead_us: int) -> None:
        ...
        if not _preview_is_playing():
            _invalidate_playback_seek(int(playhead_us))
        ...
        if (
            not _preview_is_scrubbing()
            and preview_state.get("playback_session_active")
            and not _playback_seeked_since_pause(us)
        ):
            _preview_sync_time_only(us)
            return
    ...
    def _prepare_preview_for_draft_load(content: Optional[Dict[str, Any]]) -> None:
        ...
        _maybe_resume_pending_timeline_preview()
    ...
    def open_replace_style_dialog(
        ...
        *,
        ...
        if batch_mode:
            head = (
                track_summary
                or f"[{_track_type_zh(ref.track_type)}] {ref.track_name}"
            ).strip()
            ctk.CTkLabel(
                main,
                text=head,
        ...
        *,
        ...
        for node in nodes:
            if node.is_folder:
                ...
                n_sub = count_hot_templates(children)
                pb = _make_draft_list_click_box(
                    mid,
                    node.name,
                    row_kind="parent",
    ...
    def download_template_node(node: Any) -> None:
        nonlocal selected_template_id
        from tkinter import messagebox
        ...
        if template_download_busy["on"]:
            return
        if not template_client or not auth_client or not auth_client.user_id:
            messagebox.showwarning("下载模板", "请先登录。")
            return
        base = draft_root.get().strip()
        if not base or not os.path.isdir(base):
        ...
        if local_folder:
            ...
            return
        selected_template_id = node.template_id
        template_status_var.set(f"正在导入「{node.name}」…")
        template_download_busy["on"] = True
        uid = auth_client.user_id
        tid = node.template_id
        display_name = node.name
        progress_dlg = _DraftImportProgressDialog(root, title=f"导入「{display_name}」")
        progress_dlg.update(0.0, "准备下载…")

        def _report_download(ratio: Optional[float], message: str) -> None:
            if ratio is None:
                progress_dlg.update(None, message)
            else:
                progress_dlg.update(max(0.0, min(0.65, float(ratio) * 0.65)), message)

        def _report_import(ratio: Optional[float], message: str) -> None:
            if ratio is None:
                progress_dlg.update(None, message)
            else:
                progress_dlg.update(0.65 + max(0.0, min(1.0, float(ratio))) * 0.35, message)
        ...
        def _bg() -> None:
            ...
            try:
                ...
                os.close(fd)
                hint, dl_err = template_client.download_template_to_file(
                    tid,
                    zip_path,
                    user_id=uid,
                    on_progress=_report_download,
                )
                if dl_err:
                    err = dl_err
                else:
                    _report_import(0.0, "正在导入草稿…")
                    folder_name, imp_err = import_template_zip_file(
                        base,
                        zip_path,
                        expected_folder_name=hint or display_name,
                        on_progress=_report_import,
                    )
                    if imp_err:
                        err = imp_err
            except Exception as exc:
        ...
        threading.Thread(target=_bg, daemon=True, name="hot-template-download").start()
    ...
    btn_row3 = ctk.CTkFrame(draft_panel, fg_color="transparent")
    ...
    def on_generate_sample() -> None:
        ...
        base = draft_root.get().strip()
        if not base or not os.path.isdir(base):
            messagebox.showerror("无法生成", "请先配置有效的剪映草稿根目录（com.lveditor.draft）。")
            return
    ...
    def show_draft(folder_name: str) -> None:
        ...
        prev_folder = (replace_state.get("timeline_draft_name") or "").strip()
        if prev_folder != (folder_name or "").strip():
            _cancel_cloud_preview_fetch()
        ...
        replace_state["draft_load_gen"] = int(replace_state.get("draft_load_gen") or 0) + 1
        load_gen = int(replace_state["draft_load_gen"])
        ...
        def _bg() -> None:
            ...
            try:
                payload = _load_draft_payload(dpath)
            except Exception as exc:
                err = str(exc)

            def _apply_loaded() -> None:
                ...
                raw_timeline = payload["raw_timeline"]
            ...
            try:
                root.after(0, _apply_loaded)
            except tk.TclError:
                pass

        threading.Thread(target=_bg, daemon=True, name="draft-load").start()
